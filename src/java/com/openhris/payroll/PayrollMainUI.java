        ...
        for(Payroll p : payrollList){
            if(p.getId() != 0){
                ...
                payrollTbl.addItem(new Object[]{
                    ...
                    p.getBasicSalary(), 
                    p.getHalfMonthSalary(), 
                    p.getTotalOvertimePaid(), 
                    p.getTotalLegalHolidayPaid(), 
                    p.getTotalSpecialHolidayPaid(), 
                    p.getTotalNightDifferentialPaid(), 
                    p.getTotalWorkingDayOffPaid(), 
                    p.getAbsences(), 
                    p.getTotalLatesDeduction(), 
                    p.getTotalUndertimeDeduction(), 
                    p.getGrossPay(), p.getSss(), 
                    p.getPhic(), 
                    p.getHdmf(), 
                    p.getTax(), 
                    p.getNetSalary(), 
                    p.getCashBond(), 
                    p.getAllowance(), 
                    p.getAllowanceForLiquidation(), 
                    p.getTotalAdvances(), 
                    p.getAdjustment(), 
                    p.getAmountToBeReceive(), 
                    p.getAmountReceivable(), 
                    p.getForAdjustments(), 
                    p.getPayrollPeriod(), 
